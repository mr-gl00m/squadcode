...
function safeCallIdSegment(callId: string): string {
  ...
  // Append a short hash of the FULL original id so two distinct long ids
  // sharing the first 80 sanitized characters don't collide on the same
  // artifact path.
  const cleaned = callId.replace(/[^A-Za-z0-9_.-]/g, "_");
  const prefix = cleaned.length > 0 ? cleaned.slice(0, 80) : "call";
  const disambiguator = createHash("sha256")
    .update(callId)
    .digest("hex")
    .slice(0, 12);
  return `${prefix}_${disambiguator}`;
}
...
export function artifactPath(
  ...
  return join(
    artifactDir(sessionId, baseDir),
    `${safeCallIdSegment(callId)}.txt`,
  );
}
...
export async function writeArtifact(args: {
  ...
  const buf = Buffer.from(redactedContent, "utf-8");
  const sha256 = createHash("sha256").update(buf).digest("hex");
  await atomicWriteText(path, redactedContent);
  return { path, sha256, fullSizeBytes: buf.byteLength };
}

export function tailPreview(
  content: string,
  tailBytes: number = ARTIFACT_TAIL_BYTES,
): string {
...
export function composeOffloadedContent(
  content: string,
  ref: ArtifactRef,
): string {
...
export async function maybeOffload(args: {
  ...
  try {
    ...
    return {
      content: composeOffloadedContent(args.content, artifact),
      artifact,
    };
  } catch (err: unknown) {
    logger.warn(
