...
export function toAnthropicMessages(req: CanonicalRequest): {
  ...
  for (const m of req.messages) {
    switch (m.role) {
      case "system":
        ...
        continue;
      case "user":
        // Accumulate without flushing so adjacent user-role inputs (text +
        // tool_result, or two consecutive user texts after a provider error)
        // coalesce into one Anthropic user message. pushAssistant and
        // finalize() handle the flush at the role transition.
...
export function toAnthropicTools(
  ...
  const out: AnthropicTool[] = tools.map((t) => ({
    ...
    input_schema: sanitizeToolSchema(
      t.inputSchema,
  ...
  if (cacheControlOnLast && out.length > 0) {
    ...
    (
      last as AnthropicTool & { cache_control?: { type: "ephemeral" } }
    ).cache_control = {
      type: "ephemeral",
    };
  }
