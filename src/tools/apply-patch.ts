...
export function parseUnifiedDiff(patch: string): FilePatch[] {
  ...
  while (i < lines.length) {
    ...
    if (i >= lines.length || !lines[i]!.startsWith("+++ ")) {
      throw new Error(`expected '+++ ' line after '--- ' near line ${i + 1}`);
    }
...
export const applyPatchTool = defineTool({
  ...
  execute: async (input, ctx, metadata) => {
    ...
    // Two-phase apply: stage every file's final content in memory first; only
    // write to disk if every file's hunks matched. A failure in phase 1 leaves
    // the working tree untouched, so the user can re-emit the patch against
    ...
    for (const file of parsed) {
      if (file.isNew) {
        const plan = await planNewFile(file, ctx.cwd);
      ...
      if (stats.size > MAX_FILE_BYTES) {
        return {
          ok: false,
          content: `FAIL ${file.path}: file size ${stats.size} exceeds ${MAX_FILE_BYTES}-byte ceiling`,
      ...
      if (expected !== undefined && stats.mtimeMs !== expected) {
        return {
          ok: false,
          content: `FAIL ${file.path}: mtime moved between preview and apply; re-emit the patch`,
      ...
      for (let h = 0; h < file.hunks.length; h += 1) {
        ...
        if (!out.ok) {
          return {
            ok: false,
            content: `FAIL ${file.path}: hunk ${h + 1} ${out.reason}`,
            error: "PATCH_APPLY_FAILED",
          };
        }
        working = out.content;
      ...
      planned.push({
        ...
        relPath: file.path,
        isNew: false,
        beforeContent: rawWithBom,
        finalContent,
        hunkCount: file.hunks.length,
      });
    }

    // Phase 2: every file validated. Write atomically per-file under the
    // file lock. A failure here is an OS write error, not a hunk mismatch —
    // the worktree could still be left partially mutated by the time errno
    // surfaces, but that's an OS failure mode separate from the patch
    // logic we're guarding against.
    const reports: string[] = [];
    for (const p of planned) {
      await withFileLock(p.target, async () => {
        await atomicWriteText(p.target, p.finalContent);
      });
      ctx.diagnostics?.recordTouched(p.target);
      reports.push(
        p.isNew
          ? `OK ${p.relPath}: created (${p.finalContent.split("\n").length} lines)`
          : `OK ${p.relPath}: ${p.hunkCount} hunk${p.hunkCount === 1 ? "" : "s"} applied`,
      );
    }
    ...
    return {
      ...
      mutations: planned.map((plan) => ({
        path: plan.target,
        before: plan.beforeContent,
        after: plan.finalContent,
      })),
    };
  },
...
// Plan a new-file create. Returns either a PlannedWrite shape that the
// phase-2 writer can hand to atomicWriteText, or a FAIL string the caller
// surfaces as PATCH_APPLY_FAILED. No disk writes happen here.
interface NewFilePlan {
  target: string;
  relPath: string;
  isNew: true;
  beforeContent: null;
...
async function planNewFile(
  file: FilePatch,
  cwd: string,
): Promise<NewFilePlan | string> {
